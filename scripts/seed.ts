...
import { initialState, reduce, seededRng, type Action, type State } from '../src/engine';
...
s = { ...s, gold: 420, labor: 180, energy: 90, research: 60, res: { ...s.res, iron_ore: 64, coal: 40, stone: 55, iron: 60, gravel: 22, coke: 18, silver: 3, hardened_steel: 2, precision_components: 12 } };
act({ type: 'build', building: 'furnace' }); act({ type: 'build', building: 'crusher' }); act({ type: 'build', building: 'coke_oven' });
...
act({ type: 'add_conveyor', from: { kind: 'stock' }, to: { kind: 'building', iid: f.iid }, resource: 'iron_ore', amount: 6 });
act({ type: 'add_conveyor', from: { kind: 'building', iid: f.iid }, to: { kind: 'trader' }, resource: 'iron', amount: 4 });
act({ type: 'add_conveyor', from: { kind: 'stock' }, to: { kind: 'building', iid: c.iid }, resource: 'stone', amount: 5 });
const t0 = Object.values(s.turrets)[0]!; act({ type: 'add_conveyor', from: { kind: 'stock' }, to: { kind: 'turret', iid: t0.iid }, resource: 'iron_ore', amount: 3 });
